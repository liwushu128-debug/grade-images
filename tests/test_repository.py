    ...
    def test_release_archive_is_deterministic_and_clean(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with zipfile.ZipFile(first) as archive:
                ...
                self.assertIn("grade-images/scripts/benchmark.py", names)
                self.assertIn("grade-images/scripts/route_scene.py", names)
                self.assertIn("grade-images/scripts/scene.py", names)
                self.assertIn("grade-images/scripts/correct.py", names)
                self.assertIn("grade-images/scripts/batch_render.py", names)
                self.assertIn("grade-images/scripts/v4.py", names)
                ...
                self.assertIn("grade-images/references/semantic-routing.md", names)
                self.assertIn("grade-images/references/bounded-correction.md", names)
                self.assertIn("grade-images/references/batch-consistency.md", names)
                self.assertIn("grade-images/references/v4-architecture.md", names)
