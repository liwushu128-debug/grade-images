...
- explicitly requested v0.3.4 output sharpening that derives a bounded luminance detail signal from the rendered source, with skin/noise protection and texture quality gates.
...
- blur, denoise, deblur, repair, clarity, dehaze, local-contrast enhancement, or arbitrary texture processing;
- sharpening outside the v0.3.4 `texture.mode: refine` whitelist or without an explicit user request;
...
Decode the source once, process in floating point, and encode once. Prefer a lossless PNG master. For RAW, the PNG is lossless only relative to the recorded demosaiced development, not the original sensor mosaic. A newly encoded JPEG cannot be pixel-identical to the source; label it as a lossy derivative and use high quality with minimal chroma subsampling.

## Honest limits

Do not describe tone compression as recovered detail. Fully clipped encoded pixels contain no recoverable scene information. A RAW source may contain sensor headroom absent from an accompanying JPEG, but never claim recovery beyond the RAW values or use synthetic highlight reconstruction.
